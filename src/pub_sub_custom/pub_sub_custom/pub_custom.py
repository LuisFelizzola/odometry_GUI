...
class MinimalPublisher(Node):
    def __init__(self):
        super().__init__('minimal_publisher_custom') #initialize the node class
        #create a publsiher with a String msg and the topic name "addison" with 10 of qos
        self.publisher_=self.create_publisher(TargetCoordinates, 'addison_custom',10)
    ...
    def timer_callback(self):
        ...
        msg= TargetCoordinates() #create a String msg

        #set the message's data
        msg.name="POSITION:"
        msg.coordinate.x=self.i+10.0
        msg.coordinate.y=self.i+50.0
        msg.coordinate.z=self.i+1.0
        ...
        self.get_logger().info('Publishing: "%s"  X: "%f" Y: "%f" Z: "%f"'%(msg.name, msg.coordinate.x, msg.coordinate.y ,msg.coordinate.z))
